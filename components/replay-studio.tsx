        ...
        <div className="studio-grid">
          ...
          <div className="player-surface">
            <div className="player-stage">
              {displayedVideo && !error ? <ReactPlayer className="replay-player" key={`${activeVideo ? "playback" : "preview"}-${displayedVideo.src}-${playbackNonce}`} src={displayedVideo.src} playing={activeVideo ? isPlaying : false} autoPlay={Boolean(activeVideo && isPlaying)} controls={Boolean(activeVideo)} playsInline volume={volume} width="100%" height="100%" onPlay={activeVideo ? handlePlaybackStart : undefined} onEnded={activeVideo ? handleEnded : undefined} onDurationChange={activeVideo ? (event) => setDuration(event.currentTarget.duration) : undefined} onError={activeVideo ? handlePlaybackError : () => setError("Não foi possível carregar esta URL para prévia.")} /> : <div className="player-empty"><Play aria-hidden="true" size={30} /><p>{error ? "A reprodução foi interrompida para esta fonte." : "O player aparece aqui quando a sessão começar."}</p></div>}
            </div>
